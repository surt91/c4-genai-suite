...
import { Extension, ExtensionSpec } from '../../domain/extensions';
import { GPTImage1Extension, GPTImage1ExtensionConfiguration } from './gpt-image-1';

@Extension()
export class AzureGPTImage1Extension extends GPTImage1Extension {
  get spec(): ExtensionSpec {
    const baseSpec = super.spec;
    return {
      ...baseSpec,
      name: 'azure-gpt-image-1',
      title: this.i18n.t('texts.extensions.azure-gpt-image-1.title'),
      ...
      description: this.i18n.t('texts.extensions.azure-gpt-image-1.description'),
      type: 'tool',
      arguments: {
        ...baseSpec.arguments,
        instanceName: {
        ...
        apiVersion: {
          ...
          required: true,
          format: 'select',
          examples: ['2024-02-15-preview'],
        },
      },
    };
  }

  protected createGptImageClient(configuration: AzureGPTImage1ExtensionConfiguration): AzureOpenAI {
...
export type AzureGPTImage1ExtensionConfiguration = GPTImage1ExtensionConfiguration & {
  instanceName: string;
  apiVersion: string;
